<ol class="breadcrumb">
    <li><a href="/">Dane</a></li>
    <li class="active">Punkty posiedzenia Rady Miasta</li>
</ol>

<ul class="nav nav-tabs">
    <?php foreach($modes as $key => $label) { ?>
        <li role="presentation" <?= ($mode == $key) ? 'class="active"' : ''; ?>><a href="/krakow/rada_posiedzenia_punkty_sesja?mode=<?= $key; ?>"><?= $label; ?></a></li>
    <? } ?>
</ul>
...
<ul class="list-group margin-top-5 panel-items">
    <? foreach($data as $row) { ?>
        <li class="list-group-item">
            <a href="/krakow/rada_posiedzenia_punkty_sesja/<?= $row['Punkty']['id']; ?>">
                <?= $row['Punkty']['tytul']; ?>
            </a>
        </li>
    <? } ?>
