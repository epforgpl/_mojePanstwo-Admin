...
<ul class="nav nav-tabs">
    <?php foreach($modes as $key => $label) { ?>
        <li role="presentation" <?= ($mode == $key) ? 'class="active"' : ''; ?>><a href="/krakow/posiedzenia_dzielnice?mode=<?= $key; ?>"><?= $label; ?></a></li>
    <? } ?>
</ul>
...
<ul class="list-group margin-top-5 panel-items">
    <? foreach($data as $row) { ?>
        <li class="list-group-item">
            <a href="/krakow/posiedzenia_dzielnice/view/<?= $row['RadyDzielnicePosiedzenia']['id']; ?>">
                <strong><?= $row['Dzielnice']['nazwa']; ?></strong><br/>
                <?= $this->PLText->date($row['RadyDzielnicePosiedzenia']['date']); ?>
            </a>
        </li>
    <? } ?>
</ul>
